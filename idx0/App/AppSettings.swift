...
struct NiriSettings: Codable, Equatable {
  ...
  init(from decoder: Decoder) throws {
    ...
    defaultNewColumnWidth = try Self.clampWidth(
      container.decodeIfPresent(Double.self, forKey: .defaultNewColumnWidth)
    )
    defaultNewTileHeight = try Self.clampHeight(
      container.decodeIfPresent(Double.self, forKey: .defaultNewTileHeight)
    )
  }
