...
enum WorkflowServiceError: LocalizedError {
  ...
  var errorDescription: String? {
    ...
    case let .unsupportedSchemaVersion(version):
      "Unsupported agent event schema version: \(version)"
    case .duplicateEvent:
      "Duplicate event ignored."
    case .unresolvedSession:
      "Unable to resolve target session."
    }
  }
}
...
final class WorkflowService: ObservableObject {
  ...
  func makeGitSnapshot(for session: Session) async throws -> (
    ...
    let changedFiles = await (try? gitService.diffNameStatus(path: targetPath)) ?? []
    let diffStat = await (try? gitService.diffStat(path: targetPath))
  ...
  func compareValue(for input: CompareInput) async -> (
    ...
    case let .session(sessionID):
      ...
      let gitService = GitService()
      let files = await (try? gitService.diffNameStatus(path: repoPath)) ?? []
    ...
    case let .branches(repoPath, leftBranch, rightBranch):
      let gitService = GitService()
      let files = await (try? gitService.diffNameStatus(path: repoPath, between: leftBranch, and: rightBranch)) ?? []
  ...
  func postApprovalNotificationIfNeeded(sessionID: UUID, title: String, summary: String) {
    ...
    Task { @MainActor [sessionID, title, summary] in
      ...
      let granted = await (try? notificationCenter.requestAuthorization(options: [.alert, .sound])) ?? false
